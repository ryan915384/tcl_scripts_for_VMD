...
proc angle_ryan_0913 {atomsel_list output_name frame_0 frame_1 CAH_0 CAA_0 as_ac abs_zf} {
	set outfile [open zangles-$output_name.dat w]
	for {set j $frame_0} {$j <= $frame_1} {incr j} {
		set residue_list [lsort -unique -integer [[atomselect top $atomsel_list frame $j] get residue]]
		# echo $residue_list
		foreach id_x $residue_list {
			switch $as_ac {
				asin {puts $outfile "[expr $abs_zf*[_zangle_asin $CAH_0 $CAA_0 $id_x $j]]"}
